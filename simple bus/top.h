...
SC_MODULE(simple_bus_test)
{
  ...
  {
    // Create instances
  	nb_master = new nb_master("nb master", 0x00, 500); // start from address 0x00, timeout=500ns
  ...
  ~top()
  {
    if (nb_master) {delete nb_master; nb_master = 0;}
    if (simple_bus) {delete simple_bus; simple_bus = 0;}
  }
